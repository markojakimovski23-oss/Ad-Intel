...
export default function Home() {
  ...
  const loadingMsgs = [
    'Scraping Meta Ad Library for real ads...',
    'Running AI analysis on company...',
  ...
  function md(text) {
    return text
      .replace(/\*\*(.*?)\*\*/g, '<strong style="color:#F5B900">$1</strong>')
      .replace(/\n/g, '<br/>');
  }

  const tabs = ['overview', 'pain points', 'solutions', 'outreach', 'contacts', 'competitors'];
  const painCats = ['ads', 'website', 'seo', 'social_media', 'email_marketing', 'lead_generation', 'reputation', 'content', 'competitive', 'operations'];
  const quickPrompts = [
    'Write me a full proposal',
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '40px 24px' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          ...
          <div style={{ marginBottom: 40 }}>
            <div style={{ fontSize: 11, letterSpacing: 3, color: GOLD, marginBottom: 8 }}>DIGITAL MA — AD INTEL</div>
            <h1 style={{ fontSize: 32, fontWeight: 800, margin: 0 }}>Agency Prospecting OS</h1>
            <p style={{ color: '#555', marginTop: 8, fontSize: 14 }}>Full marketing intelligence — real Meta ad data, every pain point, every opportunity</p>
          </div>
          ...
          {loading && (
            <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 12, padding: 24, marginBottom: 24, textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 12, color: '#444' }}>This may take 30-60 seconds — pulling real Meta Ad Library data</div>
              <div style={{ marginTop: 16, height: 3, background: '#1a1a1a', borderRadius: 3, overflow: 'hidden' }}>
                <div style={{ height: '100%', background: GOLD, borderRadius: 3, animation: 'progress 40s linear forwards' }} />
              </div>
            </div>
          )}
          ...
          {/* Error */}
          {error && (
            <div style={{ background: '#1a0000', border: '1px solid #ef444444', borderRadius: 10, padding: 16, color: '#ef4444', marginBottom: 24 }}>
              ⚠ {error}
            </div>
          )}
          ...
          {result && (
            <div>
              ...
              <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 14, padding: 24, marginBottom: 16 }}>
                <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start' }}>
                  <div style={{ textAlign: 'center', minWidth: 80 }}>
                    ...
                    <div style={{ fontSize: 10, color: '#555', letterSpacing: 1, marginTop: 4 }}>SIGNAL SCORE</div>
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', marginBottom: 6 }}>
                      ...
                      <button
                        ...
                        style={{ marginLeft: 'auto', background: chatOpen ? `${GOLD}22` : '#1a1a1a', border: `1px solid ${chatOpen ? GOLD : '#333'}`, color: chatOpen ? GOLD : '#888', borderRadius: 10, padding: '8px 16px', fontSize: 12, cursor: 'pointer', fontWeight: 700 }}
                      >
                        🤖 {chatOpen ? 'Close AI' : 'Ask AI Analyst'}
                      </button>
                    </div>
                    <div style={{ fontSize: 12, color: '#555', marginBottom: 10 }}>
                      {result.domain} · {result.industry} · {result.company_size} · {result.business_type} · {result.growth_stage}
                    </div>
                    <div style={{ fontSize: 13, color: '#aaa', lineHeight: 1.6, marginBottom: 14 }}>{result.signal_summary}</div>

                    {/* Stats row */}
                    <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                      <div style={{ background: '#ffffff08', borderRadius: 8, padding: '8px 14px' }}>
                        <div style={{ fontSize: 10, color: '#555', marginBottom: 2 }}>EST. REVENUE</div>
                        <div style={{ fontSize: 13, fontWeight: 700, color: GOLD }}>{result.estimated_revenue || 'Unknown'}</div>
                      </div>
                      <div style={{ background: '#ffffff08', borderRadius: 8, padding: '8px 14px' }}>
                        <div style={{ fontSize: 10, color: '#555', marginBottom: 2 }}>IN BUSINESS</div>
                        <div style={{ fontSize: 13, fontWeight: 700, color: GOLD }}>{result.years_in_business || 'Unknown'}</div>
                      </div>
                      <div style={{ background: '#ffffff08', borderRadius: 8, padding: '8px 14px' }}>
                        <div style={{ fontSize: 10, color: '#555', marginBottom: 2 }}>PLATFORMS ACTIVE</div>
                        <div style={{ fontSize: 13, fontWeight: 700, color: GOLD }}>{result.total_platforms_active || 0} / 5</div>
                      </div>
                      ...
                      <div style={{ background: result.platforms?.meta?.real_data ? '#22c55e11' : '#ffffff08', borderRadius: 8, padding: '8px 14px', border: result.platforms?.meta?.real_data ? '1px solid #22c55e33' : `1px solid ${BORDER}` }}>
                        ...
                        <div style={{ fontSize: 13, fontWeight: 700, color: result.platforms?.meta?.real_data ? '#22c55e' : '#555' }}>
                          {result.platforms?.meta?.real_data ? '✓ Real' : '~ Estimated'}
                        </div>
                      </div>
                    </div>
              ...
              <div style={{ display: 'flex', gap: 6, marginBottom: 16, flexWrap: 'wrap' }}>
                {tabs.map(t => (
                  <button key={t} onClick={() => setTab(t)} style={{ background: tab === t ? `${GOLD}22` : 'transparent', border: `1px solid ${tab === t ? `${GOLD}66` : BORDER}`, color: tab === t ? GOLD : '#555', borderRadius: 8, padding: '8px 16px', fontSize: 12, cursor: 'pointer', fontWeight: tab === t ? 700 : 400, textTransform: 'uppercase', letterSpacing: 0.5 }}>
                    {t}
                  </button>
                ))}
              </div>
                ...
                <div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 10, marginBottom: 16 }}>
                    {['meta', 'google', 'linkedin', 'tiktok', 'youtube'].map(p => (
                      <div key={p} style={{ background: CARD, border: `1px solid ${result.platforms?.[p]?.active_last_30_days ? '#22c55e33' : BORDER}`, borderRadius: 12, padding: 16, position: 'relative' }}>
                        ...
                        {result.platforms?.[p]?.last_ad_date && (
                          <div style={{ fontSize: 10, color: result.platforms?.[p]?.active_last_30_days ? '#22c55e' : '#ef4444', marginBottom: 4 }}>
                            Last: {result.platforms[p].last_ad_date}
                          </div>
                        )}
                        {/* Real data badge for Meta */}
                        {p === 'meta' && (
                        ...
                        {result.platforms?.[p]?.themes?.length > 0 && (
                          <div style={{ marginTop: 8, display: 'flex', flexWrap: 'wrap', gap: 3 }}>
                            {result.platforms[p].themes.map((t, i) => (
                              <span key={i} style={{ fontSize: 9, padding: '2px 6px', background: '#ffffff08', border: `1px solid ${BORDER}`, borderRadius: 3, color: '#777' }}>{t}</span>
                            ))}
                          </div>
                        )}
                      </div>
                  ...
                  {result.meta_ads_raw?.length > 0 && (
                    <div style={{ background: CARD, border: '1px solid #22c55e33', borderRadius: 12, padding: 20, marginBottom: 16 }}>
                      <div style={{ fontSize: 11, color: '#22c55e', letterSpacing: 1, marginBottom: 14 }}>✓ REAL META ADS FOUND ({result.meta_ads_raw.length})</div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        {result.meta_ads_raw.map((ad, i) => (
                          <div key={i} style={{ padding: '10px 14px', background: '#ffffff05', borderRadius: 8, border: `1px solid ${BORDER}` }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                              <div style={{ fontWeight: 700, fontSize: 13 }}>{ad.page || 'Unknown Page'}</div>
                              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                                {ad.started && <span style={{ fontSize: 10, color: '#555' }}>Since: {ad.started}</span>}
                                {ad.snapshot && (
                                  <a href={ad.snapshot} target="_blank" rel="noopener noreferrer" style={{ fontSize: 10, color: GOLD, textDecoration: 'none', padding: '2px 8px', border: `1px solid ${GOLD}44`, borderRadius: 4 }}>View Ad →</a>
                                )}
                              </div>
                            </div>
                            {ad.body && <div style={{ fontSize: 12, color: '#888', fontStyle: 'italic' }}>"{ad.body.slice(0, 120)}{ad.body.length > 120 ? '...' : ''}"</div>}
                            {ad.platforms && <div style={{ fontSize: 10, color: '#555', marginTop: 4 }}>Platforms: {Array.isArray(ad.platforms) ? ad.platforms.join(', ') : ad.platforms}</div>}
                          </div>
                        ))}
                      </div>
                    </div>
              ...
              {tab === 'pain points' && (
                <div>
                  <div style={{ display: 'flex', gap: 6, marginBottom: 16, flexWrap: 'wrap' }}>
                    {painCats.map(c => (
                      <button key={c} onClick={() => setPainTab(c)} style={{ background: painTab === c ? '#ef444422' : 'transparent', border: `1px solid ${painTab === c ? '#ef444466' : BORDER}`, color: painTab === c ? '#ef4444' : '#555', borderRadius: 8, padding: '6px 12px', fontSize: 11, cursor: 'pointer', fontWeight: painTab === c ? 700 : 400 }}>
                  ...
                  {result.pain_points?.[painTab]?.length > 0 ? result.pain_points[painTab].map((p, i) => (
                    <div key={i} style={{ background: CARD, border: `1px solid ${severityColor[p.severity] || BORDER}33`, borderRadius: 10, padding: '14px 18px', marginBottom: 8 }}>
                      <div style={{ display: 'flex', gap: 12 }}>
                        <span style={{ fontSize: 10, padding: '3px 8px', borderRadius: 20, background: severityBg[p.severity] || '#ffffff08', color: severityColor[p.severity] || '#888', fontWeight: 700, minWidth: 50, textAlign: 'center', flexShrink: 0 }}>{p.severity?.toUpperCase() || 'INFO'}</span>
                        <div>
                    ...
                    <div style={{ color: '#555', textAlign: 'center', padding: 40 }}>No issues detected in this category</div>
                  )}
                </div>
              )}
              ...
              {tab === 'solutions' && result.solutions && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <div style={{ background: '#ef444411', border: '1px solid #ef444433', borderRadius: 12, padding: 20 }}>
                    <div style={{ fontSize: 11, color: '#ef4444', letterSpacing: 1, marginBottom: 8 }}>🎯 TOP PRIORITY ACTION</div>
                  ...
                  {['quick_wins', 'short_term', 'long_term'].map(section => result.solutions[section]?.length > 0 && (
                    <div key={section} style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 12, padding: 20 }}>
                      <div style={{ fontSize: 11, color: GOLD, letterSpacing: 1, marginBottom: 14 }}>
                        {section === 'quick_wins' ? '⚡ QUICK WINS' : section === 'short_term' ? '📅 SHORT TERM — 30-90 Days' : '🚀 LONG TERM — 3-6 Months'}
                      </div>
                  ...
                  {result.solutions.agency_pitch && (
                    <div style={{ background: `${GOLD}11`, border: `1px solid ${GOLD}33`, borderRadius: 12, padding: 20 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                        <div style={{ fontSize: 11, color: GOLD, letterSpacing: 1 }}>💼 YOUR AGENCY PITCH</div>
                  ...
                  {result.contacts?.length > 0 ? result.contacts.map((c, i) => (
                    <div key={i} style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 10, padding: '16px 18px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16 }}>
                      ...
                      <div style={{ textAlign: 'right', flexShrink: 0 }}>
                        ...
                        {c.email && (
                          <button onClick={() => copyText(c.email, `e${i}`)} style={{ background: copied === `e${i}` ? '#22c55e22' : '#ffffff08', border: `1px solid ${copied === `e${i}` ? '#22c55e44' : BORDER}`, color: copied === `e${i}` ? '#22c55e' : '#555', borderRadius: 6, padding: '4px 12px', fontSize: 11, cursor: 'pointer' }}>
                    ...
                    <div style={{ color: '#555', textAlign: 'center', padding: 40 }}>
                      ...
                      No contacts found — add a Hunter.io API key to your .env.local for real emails
                    </div>
                  )}
                </div>
              )}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  {result.competitor_intel.competitor_advantages?.length > 0 && (
                    <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 12, padding: 20 }}>
                      ...
                      {result.competitor_intel.competitor_advantages.map((a, i) => (
                        <div key={i} style={{ fontSize: 13, color: '#aaa', padding: '8px 0', borderBottom: `1px solid ${BORDER}` }}>⚠ {a}</div>
                      ))}
                    </div>
                  )}
                  {result.competitor_intel.gaps_you_can_exploit?.length > 0 && (
                    <div style={{ background: CARD, border: '1px solid #22c55e33', borderRadius: 12, padding: 20 }}>
                      ...
                      {result.competitor_intel.gaps_you_can_exploit.map((g, i) => (
                        <div key={i} style={{ fontSize: 13, color: '#aaa', padding: '8px 0', borderBottom: `1px solid ${BORDER}` }}>✓ {g}</div>
                      ))}
                    </div>
                  )}
                </div>
      ...
      {chatOpen && result && (
        <div style={{ width: 400, minWidth: 400, background: '#0d0d0d', borderLeft: `1px solid ${BORDER}`, display: 'flex', flexDirection: 'column', height: '100vh', position: 'sticky', top: 0 }}>
          ...
          <div style={{ padding: '18px 20px', borderBottom: `1px solid ${BORDER}`, display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
            <div style={{ width: 36, height: 36, borderRadius: '50%', background: `${GOLD}22`, border: `1px solid ${GOLD}44`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, flexShrink: 0 }}>🤖</div>
            <div>
              <div style={{ fontWeight: 700, fontSize: 14 }}>AI Analyst</div>
              <div style={{ fontSize: 11, color: '#555' }}>Knows everything about {result.company_name}</div>
            </div>
            <button onClick={() => setChatOpen(false)} style={{ marginLeft: 'auto', background: 'transparent', border: 'none', color: '#444', cursor: 'pointer', fontSize: 22, lineHeight: 1, flexShrink: 0 }}>×</button>
          </div>
